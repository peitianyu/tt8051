#include "parse_split_code.h"


ParseSplitCode::ParseSplitCode() = default;

Code ParseSplitCode::get_code(const std::string& str)
{
    static std::size_t line = 0;
    return splite_code(line++, str);
}

Code ParseSplitCode::splite_code(const std::size_t line, const std::string& str)
{
    //  TODO: 调整顺序, 让查找速度更快
    ...
    // 1. 判断LABEL
    if(is_label(str)) return {line, "LABEL", get_label(str)};
    ...
    {
        if(front_str == ins) return {line, front_str, remove_spaces(back_str)};
    }
    ...
    for(auto& ins: s_special_instructions)
    {
        if(ins == back_str.substr(0, ins.size())) 
        {
            ...
            return {line, ins, splice_front_back(front_str, remove_spaces(special_data))};
        }
    } 
    
    std::cout << str << std::endl;
    return {line, "ERR", "illegal command"}; 
}

void ParseSplitCode::splite_front_back(const std::string& str, std::string& front_str, std::string& back_str)
{
...
std::string ParseSplitCode::remove_spaces(const std::string& input) 
{
    std::string output;
...
std::string ParseSplitCode::splice_front_back(const std::string& front_str, const std::string& back_str) { return front_str + ',' + back_str; }

bool ParseSplitCode::is_label(const std::string& str) { return (str.find(':') != std::string::npos); }

std::string ParseSplitCode::get_label(const std::string& str) { return str.substr(0, str.find(':')); }
